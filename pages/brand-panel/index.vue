            ...
            <ul class="grid gap-3 w-full" style="grid-template-columns: repeat(auto-fill, minmax(250px, 1fr))" v-show="!loading">
                ...
                >
                    <nuxt-link
                        ...
                        :to="localePath(`/brand-panel/${brand._id}`)"
                        :title="$t(`brand-panel.brands.Go To Dashboard`)"
                    >
                        <Icon class="w-5 h-5 bg-white" name="column-light.svg" folder="icons/light" size="20px" />
                    </nuxt-link>
                    <SlideMenu class="z-10">
                        <nuxt-link class="flex items-center gap-2 p-2 rounded-md hover:bg-dolphin" :to="localePath(`/brand-panel/${brand._id}`)">
                            <Icon class="w-4 h-4 bg-white" name="column-light.svg" folder="icons/light" size="16px" />
                            <small>{{ $t("brand-panel.brands.Go To Dashboard") }}</small>
                        </nuxt-link>
                        <nuxt-link class="flex items-center gap-2 p-2 rounded-md hover:bg-dolphin" :to="localePath(`/brand-panel/brand/${brand._id}`)">
                    ...
                    <nuxt-link class="btn" :to="localePath(`/brand-panel/${brand._id}`)" :title="$t(`brand-panel.brands.Go To Dashboard`)">
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <div class="flex items-center gap-2 w-full">
                        ...
                        <button
                            ...
                            @click="deleteBrand()"
                        >
                            <span v-if="!deleting"> {{ $t("brand-panel.brands.Delete Brand") }} </span>
...
const deleteBrand = async () => {
    ...
    await axios
        .delete(`/api/v1/brand-panel/brands/${id}`)
        .then((response) => {
