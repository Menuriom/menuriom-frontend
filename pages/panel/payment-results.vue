...
<template>
    <div class="flex flex-col items-center justify-center gap-4">
        <!-- TODO -->
        <div class="flex flex-col items-center gap-2 w-full max-w-2xl p-4 md:p-8 bg-white shadow-nr10 rounded-xl">
            ...
            <h2 class="text-2xl text-emerald-400 font-bold mt-2" v-if="route.query.statusCode == 200">{{ $t("panel.payment.Payment Successful") }}</h2>
            <h2 class="text-2xl text-rose-400 font-bold mt-2" v-else>{{ $t("panel.payment.Payment Failed") }}</h2>

            <p class="text-sm text-black opacity-80 text-center" v-if="route.query.statusCode == 200">
            ...
            <div class="flex flex-col items-center gap-1" v-else>
                <p class="text-sm text-black text-center" v-html="$t('panel.payment.errorWithCode', { code: `<b>${route.query.statusCode}</b>` })" />
        ...
        <div class="flex flex-col items-center sm:items-start gap-4 w-full max-w-2xl p-4 md:p-8 bg-white shadow-nr10 rounded-xl">
            ...
            <div
                class="flex flex-col gap-2 w-full p-2 md:p-4 border bg-pencil-tip text-white rounded-lg shadow-nr5 cursor-pointer"
            ...
            >
                ...
                <div class="dropdown-box" :class="{ open: showBillInfo }">
                    <div class="dropdown-box-inner flex flex-col gap-2">
                        <span class="opacity-90 text-sm">{{ $t("panel.payment.Bill Number") }} : {{ paymentInfo.bill.billNumber }}</span>
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-2">
                            ...
                            <small class="bg-rose-500 bg-opacity-25 text-rose-300 p-1 px-3 rounded-lg" v-if="paymentInfo.bill.status == 'notPaid'">
                                {{ $t("panel.payment.notPaid") }}
                            </small>
                            <small class="bg-blue-500 bg-opacity-25 text-blue-300 p-1 px-3 rounded-lg" v-if="paymentInfo.bill.status == 'pendingPayment'">
                                {{ $t("panel.payment.pendingPayment") }}
                            </small>
                            <small class="bg-emerald-500 bg-opacity-25 text-emerald-300 p-1 px-3 rounded-lg" v-if="paymentInfo.bill.status == 'paid'">
                                {{ $t("panel.payment.paid") }}
                            </small>
                            <small class="bg-red-500 bg-opacity-25 text-red-300 p-1 px-3 rounded-lg" v-if="paymentInfo.bill.status == 'canceled'">
                                {{ $t("panel.payment.canceled") }}
                            </small>
                        </div>
                    </div>
        ...
        <nuxt-link class="flex items-center gap-4 w-full max-w-2xl p-4 bg-white shadow-nr10 rounded-xl" :to="localePath(`/panel/${brandID}/support`)">
            <Icon class="w-10 h-10 gradient" name="square-question.svg" folder="icons/light" size="36px" />
