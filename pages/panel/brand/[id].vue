        ...
        <header class="flex flex-wrap items-center justify-between gap-4">
            <div class="flex flex-col gap-2">
                <div class="flex items-center gap-2">
                    ...
                    <h1 class="text-4xl/none font-bold">{{ $t("panel.brands.Edit Brand") }}</h1>
                </div>
            </div>
        </header>
...
const title = computed(() => `${t("panel.brands.Edit Brand")} - ${t("panel.Your Menuriom Panel")}`);
useHead({ title: title });
...
const save = async () => {
    ...
    await axios
        .post(`/api/v1/panel/brands/${route.params.id}`, data)
        .then((response) => {
            // TODO
