...
<template>
    <Dialog name="change-mobile" :title="$t('panel.profile.Change And Verify Mobile')">
        <div class="flex flex-col gap-4 md:w-screen max-w-md mt-4">
            <small class="opacity-75">{{ $t(`panel.profile.mobileVerifyMessage`) }}</small>
            <div class="flex flex-wrap sm:flex-nowrap items-start gap-2" v-if="timeLeft == 0">
                <Input
                    name="mobile"
                    mask="+98 9## ### ####"
                    class="w-full flex-grow"
                    :label="$t('panel.profile.Phone Number')"
                    :required="true"
                    v-model="mobile"
...
const sendCode = async () => {
    ...
    await axios
        .post(`/api/v1/user/change-mobile`, { mobile: useNumbersToEnglish(mobile.value.replaceAll(" ", "")) })
        .then((response) => {
...
const verify = async () => {
    ...
    await axios
        .post(`/api/v1/user/verify-mobile`, { code: useNumbersToEnglish(code.value) })
        .then((response) => {
            userStore.mobile = response.data.mobile;
            toast.info(t(`panel.profile.Your phone number has been updated`), { timeout: 3000, rtl: localeProperties.value.dir == "rtl" });
            timeLeft.value = 0;
            code.value = "";
