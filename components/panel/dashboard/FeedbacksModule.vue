...
<template>
    <section class="flex flex-col gap-4 w-full p-6 bg-bgAccent rounded-3xl">
        ...
        <ul class="flex flex-col gap-4 w-full max-h-64 overflow-auto pe-2">
            <li class="flex items-start gap-2 group" v-for="i in 5">
                <div class="flex flex-col gap-2 shrink-0">
                    ...
                    <nuxt-link class="flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all text-xs/none bg-primary text-white p-2 rounded-lg shadow-mr35" to="">
                        {{ $t("panel.dashboard.Reply") }}
                    </nuxt-link>
                </div>
                <div class="flex flex-col gap-1">
                    <span class="text-primary font-bold">Food name</span>
                    <div class="flex flex-col gap-1 w-full min-h-[36px] p-2 rounded-se-2xl rounded-b-2xl bg-bgSecondary bg-opacity-80 shadow-mr25">
                        <div class="flex items-center justify-between gap-2 text-sm">
                            <span>user's name</span>
                            <small class="text-secondary opacity-90">date</small>
                        </div>
                        <p class="text-xs opacity-75">I'm really satisfied with the performance of this product. It has made my life much easier.</p>
                    </div>
                </div>
            </li>
        </ul>
