        ...
        >
            <template #item="{ element: groups, index: k }">
                <div class="flex flex-col gap-3">
                    <div class="flex items-center gap-1 max-w-max p-2 bg-white rounded-md shadow-nr10">
                        <Icon class="group_grab_area w-5 h-5 bg-black rotate-90" name="grip-dots.svg" folder="icons/light" size="20px" />
                        <img class="h-6 grayscale" :src="categories.list[k].icon" alt="" />
                        <h3 class="font-semibold">
                            {{ dishes.list[k][0].category.translation?.[locale]?.name || dishes.list[k][0].category.name }}
                        </h3>
                    </div>
                    <small class="opacity-75" v-if="!dishes.list[k][1]">{{ $t("panel.menu.This Category Has No Items") }}</small>

                    <Draggable
                        tag="ul"
                        class="flex flex-col gap-3 w-full ps-3 border-s-2 border-neutral-200"
                        v-model="dishes.list[k]"
                        @start="resetSavingOrder()"
                        @end="saveItemOrder()"
                        handle=".grab_area"
                        item-key="order"
