    ...
    <Wrapper class="flex flex-col gap-2 w-full p-4 bg-white rounded-lg shadow-nr5" :defaultState="0">
        <template #title>
            <div class="flex items-center justify-between gap-2 w-full">
                <h3 class="font-bold">{{ $t("panel.menu-style.Categories Section") }}</h3>
        ...
        <div class="flex flex-col gap-4 overflow-hidden">
            <div class="flex flex-wrap items-center gap-2">
                <h5>{{ $t("panel.menu-style.Orientation") }}</h5>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <input class="input-range light w-40" type="range" max="50" step="5" v-model="categoriesOptions.cornerRadius" />
            </div>
             <div class="flex flex-wrap items-center gap-2">
                <h5>{{ $t("panel.menu-style.Show categories icons") }}</h5>
                <Switch_Light v-model:value="categoriesOptions.withIcon" />
            </div>
