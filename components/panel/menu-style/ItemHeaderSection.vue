    ...
    <Wrapper class="flex flex-col gap-2 w-full p-4 bg-white rounded-lg shadow-nr5" :defaultState="0">
        <template #title>
            <div class="flex items-center justify-between gap-2 w-full">
                <h3 class="font-bold">{{ $t("panel.menu-style.Items Title Section") }}</h3>
        ...
        <div class="flex flex-col gap-4 overflow-hidden">
            <div class="flex flex-wrap items-center gap-2">
                <h5>{{ $t("panel.menu-style.Items Title Type") }}</h5>
                <ul class="flex flex-wrap items-center gap-2">
                    <li
                        ...
                        :class="{ 'bg-pencil-tip text-white': itemHeaderOptions.component == item }"
                        @click="itemHeaderOptions.component = item"
                        v-for="item in itemHeaderOptions.componentList"
                    >
                        <span class="flex items-center justify-center w-5 h-5 p-0.5 rounded-full border-2 border-neutral-500 border-opacity-50">
                            <b
                                ...
                                :class="{ 'bg-primary shadow-xl shadow-primary': itemHeaderOptions.component == item }"
                            ></b>
                        </span>
            ...
            <div class="flex flex-wrap items-center gap-2">
                <h5>{{ $t("panel.menu-style.Show categories icons") }}</h5>
                <Switch_Light v-model:value="itemHeaderOptions.withIcon" />
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <h5>{{ $t("panel.menu-style.Corner Radius") }}</h5>
                <input class="input-range light w-40" type="range" max="20" step="2" v-model="itemHeaderOptions.cornerRadius" />
            </div>
            <div class="flex flex-col gap-2">
                <div class="flex flex-wrap gap-2">
                    <ColorPicker :label="$t('panel.menu-style.text color')" v-model:color="itemHeaderOptions.textColor" :baseColor="baseColors.textColor" />
                    <ColorPicker
                        :label="$t('panel.menu-style.background main')"
                        v-model:color="itemHeaderOptions.bgMainColor"
                        :baseColor="baseColors.bgMainColor"
                    />
                    <ColorPicker
                        :label="$t('panel.menu-style.background accent')"
                        v-model:color="itemHeaderOptions.bgSecondaryColor"
                        :baseColor="baseColors.bgSecondaryColor"
                    />
                    <ColorPicker
                        :label="$t('panel.menu-style.accent color')"
                        v-model:color="itemHeaderOptions.accentColor"
