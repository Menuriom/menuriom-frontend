    ...
    <Wrapper class="flex flex-col gap-2 w-full p-4 bg-white rounded-lg shadow-nr5" :defaultState="1">
        ...
        <div class="flex flex-col gap-4 overflow-hidden">
            <div class="flex flex-wrap items-center gap-2">
                <h5>{{ $t("panel.menu-style.Items Style") }}</h5>
                    ...
                    >
                        <span class="flex items-center justify-center w-5 h-5 p-0.5 rounded-full border-2 border-neutral-500 border-opacity-50">
                            <b
                                ...
                                :class="{ 'bg-violet shadow-xl shadow-violet': itemListOptions.component == item }"
                            ></b>
                        </span>
            ...
            <div class="flex flex-wrap items-center gap-2" v-if="itemListOptions.component === 'ItemList3'">
                <h5>{{ $t("panel.menu-style.Show items in zig-zag") }}</h5>
                <Switch_Light v-model:value="itemListOptions.zigzag" />
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <h5>{{ $t("panel.menu-style.Corner Radius") }}</h5>
                <input class="input-range light w-40" type="range" max="24" step="2" v-model="itemListOptions.cornerRadius" />
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <h5>{{ $t("panel.menu-style.Thumbnail Overflow") }}</h5>
                <input class="input-range light w-40" type="range" max="20" step="5" v-model="itemListOptions.imageMargin" />
            </div>
            <div class="flex flex-wrap lg:flex-nowrap gap-4 w-full">
                <div class="flex flex-col gap-2 p-2 w-full rounded-lg border borde-neutral-500 border-opacity-50">
                    <div class="flex flex-wrap items-center gap-2">
                        <div class="flex items-center gap-2">
                            ...
                            <Switch_Light v-model:value="itemListOptions.withPattern" />
                        </div>
                        ...
                        <div class="flex flex-col justify-center gap-4 w-full h-28" v-show="patternMode === 'list'">
                            <ul class="flex flex-wrap items-start gap-2 w-full h-full overflow-auto">
                                <li
                                    ...
                                    :class="[itemListOptions.bgImage === pattern ? 'border-violet' : 'border-transparent']"
                ...
                <div class="flex flex-col gap-2" v-if="itemListOptions.withPattern">
                    <div class="flex flex-col items-start gap-1">
                        ...
                        <input class="input-range light w-44" type="range" min="10" max="100" step="10" v-model="itemListOptions.bgImageOpacity" />
                    </div>
                    <div class="flex flex-col items-start gap-1">
                        <label class="text-sm">{{ $t("panel.menu-style.Pattern Rotations") }}</label>
                        <input class="input-range light w-44" type="range" min="-180" max="180" step="5" v-model="itemListOptions.bgImageRotation" />
                    </div>
                    <div class="flex flex-col items-start gap-1">
                        <label class="text-sm">{{ $t("panel.menu-style.Pattern Size") }}</label>
                        <input class="input-range light w-44" type="range" min="10" max="100" step="10" v-model="itemListOptions.bgImageSize" />
                    </div>
                </div>
            </div>
