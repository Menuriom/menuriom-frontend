    ...
    <Wrapper class="flex flex-col gap-4 w-full p-4 bg-bgAccent rounded-2xl shadow-mr15" :defaultState="0">
        <template #title>
            <div class="flex items-center justify-between gap-2 w-full group">
                <h3 class="font-bold text-primary">{{ $t("panel.menu-style.Navbar Section") }}</h3>
        ...
        <div class="flex flex-col gap-6 overflow-hidden">
            <div class="flex flex-wrap items-center gap-2">
                <h5 class="text-sm opacity-80">{{ $t("panel.menu-style.Navbar Type") }}</h5>
                <ul class="flex flex-wrap items-center gap-2">
                    <li
                        class="flex items-center gap-2 p-1.5 rounded-full border border-bgSecondary shadow-nr15 cursor-pointer"
                        :class="{ 'bg-bgSecondary': navbarOptions.component == item }"
                    ...
                    >
                        <span class="flex items-center justify-center w-5 h-5 p-0.5 rounded-full border-2 border-neutral-500 border-opacity-50">
                            <b
                                ...
                                :class="{ 'bg-primary shadow-xl shadow-primary': navbarOptions.component == item }"
                            ></b>
                        </span>
            ...
            <div class="flex flex-wrap items-center gap-2">
                <h5 class="text-sm opacity-80">{{ $t("panel.menu-style.Show text") }}</h5>
                <Switch v-model:value="navbarOptions.withText" />
            </div>
            <div class="flex flex-wrap items-center gap-2" v-if="navbarOptions.component != 'Navbar1'">
                ...
                <input class="input-range w-40" type="range" max="50" step="5" v-model="navbarOptions.radius" />
            </div>
            <hr class="w-full border-bgSecondary" />
