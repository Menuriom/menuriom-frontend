...
<template>
    <div class="relative flex justify-center items-end w-full max-w-screen-4xl px-2 md:px-6 -mt-2">
        <section
            class="relative flex flex-wrap 1.5xl:flex-nowrap items-center justify-center 1.5xl:justify-between gap-10 w-full p-4 py-10 sm:p-20 lg:py-10 bg-neutral-50 text-bgPrimary rounded-3xl overflow-hidden isolate"
            title="The Best Way To Manage Your Restaurant Menu And Orders"
        >
            <NuxtImg class="absolute inset-0 -z-10 opacity-10" src="/img/restaurant.jpg" sizes="100% 1920px" />
            <div class="relative flex flex-col items-center 1.5xl:items-start gap-6 md:gap-10 w-full max-w-6xl bg-neutral-5000 isolate">
                <div
                    class="bar absolute w-full h-16 md:h-24 rounded-xl bg-gradient-to-r from-secondary to-primary opacity-50 -z-10"
                ...
                <h1 class="relative flex flex-col items-center 1.5xl:items-start text-4xl/snug sm:text-5xl/snug md:text-7xl/snug font-extrabold">
                    {{ $t("home.The Best Way To Manage Your Restaurant Menu And Orders") }}
                </h1>
                <div class="relative flex flex-col gap-6 w-full bg-bgPrimary text-fgPrimary rounded-2xl p-6 sm:p-8 shadow-mr15 isolate">
                    <div class="absolute inset-0 -z-10 pattern opacity-10"></div>
                    <p class="text-center sm:text-start text-xl md:text-2xl/relaxed font-bold opacity-90">
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-3 sm:gap-5 sm:mt-4">
                        ...
                        >
                            <span class="f-inter md:text-xl font-bold">{{ $t("home.Try It For Free") }}</span>
                        </nuxt-link>
                        <nuxt-link class="p-4 px-6 hover:px-8 rounded-xl bg-primary text-fgPrimary transition-all shadow-mr25" to="/panel" v-else>
                            <span class="f-inter md:text-xl font-bold">{{ $t("home.Head To Dashboard") }}</span>
                        </nuxt-link>
                        <span class="hidden sm:inline-block">{{ $t("auth.OR") }}</span>
                        <nuxt-link
                            class="p-4 px-6 hover:px-8 rounded-xl bg-fgPrimary text-bgPrimary transition-all shadow-mr25"
                            to="/demo"
                            v-if="!user.email.value"
                        >
                            <span class="f-inter md:text-xl font-bold">{{ $t("home.Check The Demo") }}</span>
                        </nuxt-link>
                        <nuxt-link class="p-4 px-6 hover:px-8 rounded-xl bg-fgPrimary text-bgPrimary transition-all shadow-mr25" to="/help-center" v-else>
                            <span class="f-inter md:text-xl font-bold">{{ $t("home.Learn How It Works") }}</span>
                        </nuxt-link>
                    </div>
                </div>
